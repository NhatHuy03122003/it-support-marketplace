...
import type { Message, Conversation } from "../../../../types/chatting";
...
const ChatArea = ({
  ...
  }, [messages.length, activeConv._id]);
  return (
          ...
          >
            {messages.map((msg: Message) => (
              <motion.div
                ...
                className={`flex items-end gap-3 max-w-[90%] lg:max-w-[80%] ${isMe(msg.sender._id) ? "ml-auto flex-row-reverse" : ""
                  }`}
              >

                <div
                  className={`space-y-1 ${isMe(msg.sender._id) ? "text-right" : ""
                    }`}
                >
                  <div
                    className={`p-4 text-sm leading-relaxed shadow-sm ${isMe(msg.sender._id)
                  ...
                  <div
                    className={`flex items-center gap-1 mt-1 ${isMe(msg.sender._id) ? "justify-end" : ""
                      }`}
                  >
                    <span className="text-[10px] text-slate-400 font-medium">
                      {formatDatetoTextFromNow(msg.createdAt)}
                    </span>
                    {isMe(msg.sender._id) && (
                      <CheckCircle2 color="#fff" className="w-4 h-4 text-blue-600 fill-blue-600" />
                    )}
                  </div>
                </div>
              </motion.div>
